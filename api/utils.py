...
def getNextStage(order_id, order_work_label):
    ...
    exclude_list = ["aari_work","embroidery","cutting_stiching","cutting", "stitching", "hook", "overlock", "Completed"]
    for i in s:
        if i.assign_stage in exclude_list:
            exclude_list.remove(i.assign_stage)
        else:
            pass
    include_list = ["aari_work","embroidery","cutting_stiching","cutting", "stitching", "hook", "overlock", "Completed"]
    finish_list = []
    for i in s:
    ...
    if finish_list != []:
        for idx in range(0, n, 1):
            include_res_mob.append({k[0]: finish_list[idx],k[1]: finish_list[idx]})
    ...
    if exclude_list != []:
        for idx in range(0, n, 1):
            exclude_res_mob.append({k[0]: exclude_list[idx],k[1]: exclude_list[idx]})

    if exclude_list != []:
        data = {"nextassign": exclude_res, "finishedassign": include_res,"nextassign_list":exclude_res_mob,"finishedassign_list":include_res_mob}
        return data
    else:
        data = {"nextassign": exclude_res, "finishedassign": include_res,"nextassign_list":exclude_res_mob,"finishedassign_list":include_res_mob}
        return data
